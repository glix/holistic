...
function compare_heading($data = array()){
...
function compare_body($data =array()){
	foreach ($data as $key => $value) {
		echo "<td>";
		echo $value;
		echo "</td>";
...
{
    wp_enqueue_style('my_meta_css', THEME_PATH . '/custom/meta.css');
    foreach (array('product_manager') as $type){
        add_meta_box('product_manager', 'Product Manager Fields', 'my_meta_setup', $type, 'normal', 'core');
    }
